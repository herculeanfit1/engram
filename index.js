...
pool.connect()
  .then(client => { client.release(); console.log('PostgreSQL connected'); })
  ...
  try {
    ...
    for (const thought of thoughts) {
      try {
        ...
        const metadata = thought.metadata || { type: 'imported', topics: [], people: [], action_items: [] };
        ...
        results.push({ content: thought.content.substring(0, 50), status: 'error', error: err.message });
        failed++;
      }
    }
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'ok', service: 'engram', version: '1.0.0', database: 'connected' });
  } catch (error) {
    res.status(500).json({ status: 'error', service: 'engram', version: '1.0.0', database: 'disconnected', error: error.message });
  }
});
